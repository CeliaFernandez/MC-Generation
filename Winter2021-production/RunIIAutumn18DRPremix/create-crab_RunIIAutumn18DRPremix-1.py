...
config.General.requestName = 'crab_NLO_HToSSTo4l_MHMASSHIGGS_MSMASSX_ctauSCTAU_13TeV-step1'
...
config.JobType.psetName = 'EXO-RunIIAutumn18DRPremix-1_cfg.py'
...
config.Data.outputDatasetTag = 'RunIIAutumn18DRPremix-1'
...
def makeFile(higgsmass, xmass, ctau, inputdataset, nsplitting):
    ...
    name = 'crab_NLO_HToSSTo4l_MH{0}_MS{1}_ctauS{2}_13TeV-step1.py'.format(higgsmass, xmass, ctau)
    f = open(name, 'w')
    f.write(text + '\n')
    f.close()
